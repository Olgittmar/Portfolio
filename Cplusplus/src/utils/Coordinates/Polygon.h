...
class Polygon {
  public:
    Polygon() = default;
    ~Polygon() = default;
    explicit Polygon(const std::string& str);
    ...
    std::string classify( const Point p ) const;

    static void readPolygon( std::istream& in, const int numLines, Polygon& out );
    friend ostream& operator<<( std::ostream& os, const Polygon& poly);
    ...
    struct _Iterator {
        ...
        explicit _Iterator( const std::vector<Point>& vertices, const std::vector<Point>::const_iterator& it )
      ...
      protected:
        std::vector<Point>::const_iterator m_it;
        const std::vector<Point>& _container;
    };
